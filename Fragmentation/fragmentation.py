...
def demise_components(titan, i, joints_id, options): 
    ...
    for id in joints_id:
        ...
        if options.qoi.flag: 
            if titan.assembly[i].objects[id-1].name in options.qoi.stls:
    ...
    for enum_obj, obj in enumerate(titan.assembly[i].objects):
        if obj.mass <= 0 or titan.assembly[i].trajectory.altitude <= 0:
            ...
            if options.qoi.flag: 
                if obj.name in options.qoi.stls: write_demise(titan.assembly[i],obj,options)
...
def fragmentation(titan, options):
    ...
    for it in range(lenght_assembly):
        ...
        if len(objs_id) != 0 or primitive_separation:
            ...
            elif options.qoi.flag:
                if titan.assembly[it].objects[0].name in options.qoi.stls:
