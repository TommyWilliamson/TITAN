...
class sigmaPosition(MutableSequence):
    ...
    def write_data(self,options,assem_ID,time):
        ...
        for i_point in range(7):
            # ned = pymap3d.ecef2geodetic(self.sigmas[i_point][0], self.sigmas[i_point][1], self.sigmas[i_point][2],
            ...
            for iax, ax in enumerate(['x','y','z']):
                ...
                data_sp.append(self.sigmas[i_point][iax])

        [columns.append('Cov_'+str(i)) for i in range(9)]
        data=pd.DataFrame(np.hstack(([[time]],[data_sp],data_cov)),columns=columns)
        doHeader = False if os.path.exists('utstats_{}.csv'.format(assem_ID)) else True
        data.to_csv('utstats_{}.csv'.format(assem_ID),mode='a',index=False,header=doHeader)
