...
def write_demise(assembly,object,options):
    ...
    lookup={'Assembly_ID':assembly.id,'Mass':assembly.mass,'Altitude':assembly.trajectory.altitude,'Velocity':assembly.trajectory.velocity,
        ...
        'Quat_y': assembly.quaternion[1],'Quat_z': assembly.quaternion[2],'Mach':assembly.freestream.mach,'Speedsound':assembly.freestream.sound,
    ...
    msg.read_data()
    with open(options.qoi.filepath,'rb') as file: options.qoi.quantities=pickle.load(file)

    stl = object.name
    msg.print_n_send('Tracked object \'{}\' has demised at altitude {}'.format(stl,assembly.trajectory.altitude))
    for output_name, _ in options.qoi.quantities[stl].items():
        options.qoi.quantities[stl][output_name].append(lookup[output_name])

    with open(options.qoi.filepath,'wb') as file: pickle.dump(options.qoi.quantities,file)
